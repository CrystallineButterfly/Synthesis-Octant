...
contract OctantSignalForgeTest is Test {
    OctantSignalForge private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_octant");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new OctantSignalForge(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        OctantSignalForge freshController = new OctantSignalForge(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
