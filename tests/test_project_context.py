...
MODULE = importlib.import_module('agents.octant_signal_forge')
...
class ProjectContextTest(unittest.TestCase):
    ...
    def test_primary_names_match(self) -> None:
        spec = MODULE.build_project_spec()
        self.assertEqual(spec.primary_python_module, 'octant_signal_forge')
        self.assertEqual(spec.primary_contract_name, 'OctantSignalForge')
